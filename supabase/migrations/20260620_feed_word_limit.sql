-- Social feed word limits (anti-spam).
-- Adds a maximum word count to posts (300) and comments (100), enforced inside
-- the RPCs so it cannot be bypassed by the client. Existing character caps and
-- profanity moderation are preserved unchanged.
...
BEGIN
  ...
  IF v_body <> '' AND array_length(regexp_split_to_array(v_body, '\s+'), 1) > 300 THEN
    RAISE EXCEPTION 'Post is too long (max 300 words)';
  END IF;
...
BEGIN
  ...
  IF array_length(regexp_split_to_array(v_body, '\s+'), 1) > 100 THEN
    RAISE EXCEPTION 'Comment is too long (max 100 words)';
  END IF;
